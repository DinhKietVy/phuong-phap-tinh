...
double sai_so(double x1, double x2,double bien, double pt[], int n,double a, double b) {
	...
	if((dao_ham1(pt,n,a)*dao_ham1(pt,n,b)>0) && m_nho >= 0) {
		ss = ((m_lon-m_nho)*(abs(x2-x1)))/m_nho;
	}
	else {
...
int main() {
	double a,b,c,bien,fa,fb,x,x1,x2,saiso,chon;
	...
	if(tinh_fx(a, bien, pt, n)*tinh_fx(b, bien, pt, n)>0 ||  dao_ham1(pt, n, a)<0 && dao_ham1(pt, n, b)>0 || dao_ham1(pt, n, a)>0 && dao_ham1(pt, n, b)<0 || a>b) {
		...
		// printf("chon diem bat dau: ");
		// scanf("%lf",&chon);
        if(tinh_fx(b,bien,pt,n)*dao_ham2(pt,n,a)<0) {
            x = b;
			while(saiso<saisotest) {
				x1=x;
				x = x - ((tinh_fx(x, bien, pt, n)*(a-x))/(tinh_fx(a,bien,pt,n) - tinh_fx(x, bien, pt, n)));
				count++;
				printf("buoc %d x= %lf\n",count, x);
				saisotest= abs(sai_so(x1,x,bien,pt,n,a,b));
			}
        }
        else if(tinh_fx(a,bien,pt,n)*dao_ham2(pt,n,a)<0) {
            x = a;
            while(saiso<saisotest) {
				x1=x;
				x = x - ((tinh_fx(x, bien, pt, n)*(b-x))/(tinh_fx(b,bien,pt,n) - tinh_fx(x, bien, pt, n)));
				count++;
				printf("buoc %d x= %lf\n",count, x);
				saisotest= abs(sai_so(x1,x,bien,pt,n,a,b));
			}
